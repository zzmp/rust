...
pub fn render(input: &str, mut output: Path, matches: &getopts::Matches, external_html: &ExternalHtml)
        -> int {
    ...
    let (in_header, before_content, after_content) =
        match (external_html.in_header,
               external_html.before_content,
               external_html.after_content) {
